...
async def get_username_for_add_allowed_user_rout(
    ...
    try:
        ...
        future_allowed_user_username: str = (
            message.text.strip() if message.text[0] != "@" else message.text[1:].strip()
        )
        ...
        await add_allowed_user(
            message=message, user_username=user_username, user_id=user_id
        )
